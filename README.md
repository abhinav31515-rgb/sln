...
When deploying to production, run the following commands in order:
...
To verify system health (database, cache, and queue connectivity):

```bash
php artisan app:health-check
```

This command returns a non-zero exit code if any check fails, making it suitable for monitoring and deployment pipelines.
